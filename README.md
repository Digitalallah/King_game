...
- выбор карты первым тапом и ход повторным тапом по поднятой карте;
- адаптивное вписывание в экран, включая горизонтальную ориентацию телефона;
- встроенные окна «Правила» и «Об игре»;
- запуск в обычном браузере и Telegram Mini App;
- автоматический пропуск газетной заставки и служебных запросов при старте;
- замедленный темп оригинальных анимаций.
...
Smoke-тест запускает настоящий `KING.EXE`, проходит стартовые запросы, выбирает партнёров и делает реальный ход картой в разрешении 640×350.
